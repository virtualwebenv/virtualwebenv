...
For information on how to configure xDebug with your IDE and work it out, check this [Repository](https://github.com/LarryEitel/laravel-VirtualWebEnv-phpstorm) or follow up on the next section if you use linux and PhpStorm.
...
To control the behavior of xDebug (in the `php-fpm` Container), you can run the following commands from the VirtualWebEnv root folder, (at the same prompt where you run docker-compose):
...
<br>
<a name="VirtualWebEnv-for-Production"></a>
## Prepare VirtualWebEnv for Production
...
By default, VirtualWebEnv assumes the Laravel application is living in the parent directory of the VirtualWebEnv folder.
...
5 - Go back to the VirtualWebEnv installation steps to see how to edit the `.env` file.
...
Note: Should add `--user=VirtualWebEnv` (example `docker-compose exec --user=VirtualWebEnv workspace bash`) to have files created as your host's user to prevent issue owner of log file will be changed to root then laravel website cannot write on log file if using rotated log and new log file not existed
...
VirtualWebEnv provides 2 ways to run Laravel Scheduler
1. Using cron in workspace container. 
Most of the time, when you start VirtualWebEnv, it'll automatically start workspace container with cron inside, along with setting to run `schedule:run` command every minute.
...
This way is suggested when you don't want to start workspace in production environment.
   * Comment out cron setting in workspace container, file `workspace/crontab/VirtualWebEnv`
     ```bash
     # * * * * * VirtualWebEnv /usr/bin/php /var/www/artisan schedule:run >> /dev/null 2>&1
      ```
...
MAILU_DOMAIN=VirtualWebEnv.io
MAILU_HOSTNAMES=mail.VirtualWebEnv.io
```
    ...
    ```yml
        gitlab-runner:
          environment: # these values will be used during `gitlab-runner register`
            ...
            - DOCKER_IMAGE=alpine
            - DOCKER_NETWORK_MODE=VirtualWebEnv_backend
          networks:
...
To install CodeIgniter 3 on VirtualWebEnv all you have to do is the following simple steps:
...
You can add your cron jobs to `workspace/crontab/VirtualWebEnv` after the `php artisan` line.
...
ssh -o PasswordAuthentication=no    \
    ...
    VirtualWebEnv@localhost
```

To login as root, replace VirtualWebEnv@localhost with root@localhost.
...
A `.npmrc` file is included in the `workspace` folder if you need to utilise this globally. This is copied automatically into the root and VirtualWebEnv user's folders on build.
...
When you start your docker container, VirtualWebEnv will copy the `aliases.sh` file located in the `VirtualWebEnv/workspace` directory and add sourcing to the container `~/.bashrc` file.
...
5 - Use it `docker-compose exec --user=VirtualWebEnv workspace zsh`

**Note** You can configure Oh My ZSH by editing the `/home/VirtualWebEnv/.zshrc` in running container.
...
VirtualWebEnv provides aliases through the `aliases.sh` file located in the `VirtualWebEnv/workspace` directory. You can load it into ZSH.
...
<br>
<a name="keep-tracking-VirtualWebEnv"></a>
## Track your VirtualWebEnv changes

1. Fork the VirtualWebEnv repository.
...
You can use the d4m-nfs solution in 2 ways, the first is by using the built-in VirtualWebEnv integration, and the second is using the tool separately. Below is show case of both methods:
...
VirtualWebEnv comes with `sync.sh`, an optional bash script, that automates installing, running and stopping docker-sync.  Note that to run the bash script you may need to change the permissions `chmod 755 sync.sh`

1) Configure your VirtualWebEnv environment as you would normally do and test your application to make sure that your sites are running correctly.
...
./sync.sh install
```
5) Start docker-sync and the VirtualWebEnv environment.
...
```bash
alias devup="cd /PATH_TO_virtualwebenv/virtualwebenv; ./sync.sh up nginx mysql" #add your services
alias devbash="cd /PATH_TO_virtualwebenv/virtualwebenv; ./sync.sh bash"
alias devdown="cd /PATH_TO_virtualwebenv/virtualwebenv; ./sync.sh down"
```
...
- You may run VirtualWebEnv with or without docker-sync at any time using with the same `.env` and `docker-compose.yml`, because the configuration is overridden automatically when docker-sync is used.
- You may inspect the `sync.sh` script to learn each of the commands and even add custom ones.
- If a container cannot access the files on docker-sync, you may need to set a user on the Dockerfile of that container with an id of 1000 (this is the UID that nginx and php-fpm have configured on VirtualWebEnv). Alternatively, you may change the permissions to 777, but this is **not** recommended.
...
<br>
<a name="upgrade-VirtualWebEnv"></a>
## Upgrade VirtualWebEnv

Moving from Docker Toolbox (VirtualBox) to Docker Native (for Mac/Windows). Requires upgrading VirtualWebEnv from v3.* to v4.*:

1. Stop the docker VM `docker-machine stop {default}`
2. Install Docker for [Mac](https://docs.docker.com/docker-for-mac/) or [Windows](https://docs.docker.com/docker-for-windows/).
3. Upgrade VirtualWebEnv to `v4.*.*` (`git pull origin master`)
4. Use VirtualWebEnv as you used to do: `docker-compose up -d nginx mysql`.
