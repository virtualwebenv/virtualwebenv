...
Join the chat room on [Gitter](https://gitter.im/lt/) and get help and support from the community.
...
You can as well can open an [issue](https://github.com/virtualwebenv/virtualwebenv/issues) on Github (will be labeled as Question) and discuss it with people on [Gitter](https://gitter.im/virtualwebenv/virtualwebenv/).
...
* Option B
   1. Change the `DB_HOST` value to the same name as the MySQL docker container. The VirtualWebEnv docker-compose file currently has this as `mysql`

## I get stuck when building nginx on (fetch mirrors.aliyun.com/alpine/v3.5/main/x86_64/APKINDEX.tar.gz)

As stated on [#749](https://github.com/virtualwebenv/virtualwebenv/issues/749#issuecomment-419652646), Already fixed，just set `CHANGE_SOURCE` to false.
